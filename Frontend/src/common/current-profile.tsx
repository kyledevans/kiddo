import { createContext, useState, Dispatch, SetStateAction, FunctionComponent, useEffect, useContext, ComponentType, useCallback, ReactNode } from "react";
...
function useInitializationEffect(
  ...
  authManager: IAuthenticationManager | null,
  authState: AuthenticationManagerStateType | null,
  setCurrentProfile: Dispatch<SetStateAction<Profile | null | "Anonymous" | "Unregistered">>,
) {
  useEffect(() => {
    (async () => {
      if (authState === AuthenticationManagerStateType.Running) {
        if (authManager == null)
          throw new Error('Authentication manager state cannot be "Running" if the authentication manager instance is itself null or undefined.');
...
export const AppCurrentProfileContextProvider: FunctionComponent<{ children?: ReactNode }> = ({ children }) => {
...
export type CurrentProfileContextType = [
  currentProfile: Profile | null | "Anonymous" | "Unregistered",
  setCurrentProfile: Dispatch<SetStateAction<Profile | null | "Anonymous" | "Unregistered">>,
];
