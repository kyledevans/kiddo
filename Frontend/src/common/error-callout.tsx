import { useCallback, useEffect, useRef, FunctionComponent, MutableRefObject, ReactNode } from "react";
...
export const ErrorCallout: FunctionComponent<{ target: Target; control: MutableRefObject<ErrorCalloutControl | null>; children?: ReactNode }> = ({
  target,
  control,
  children,
}) => {
  ...
  return (
    <>
      {isVisible && (
        <Callout dismissOnTargetClick={true} target={target} onDismiss={setIsHidden} onClick={setIsHidden} className={errorCalloutStyles.callout}>
          {children}
          <Icon className={errorCalloutStyles.errorIcon} iconName={errorCalloutIcon.iconName} />
        </Callout>
      )}
    </>
  );
